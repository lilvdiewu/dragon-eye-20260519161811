...
已验证字段（2026-05-19）:
  tdxstat.cfg:
    ...
    [9] 市盈率(动)                      ⚠️ (部分值偏低需验证)
    [10] 市净率                         ⚠️
    [11] 流通市值(万元)                  ⚠️ (值偏低，可能单位不同)
    [14] 总市值(万元)                    ⚠️ (值偏低，可能单位不同)
    [15] 成交量(手)                     ✅
    [22] 未知(可能是成交笔数/委比)        ❌ 不是成交额！值太小
    [23] 板块上涨家数                    ⚠️
    [24] 板块总成交额                    ⚠️
    [17]-[21] 阶段涨跌幅(20/60/120/250/YTD) ✅ (值合理)
    [27]-[30] 阶段涨跌幅(较长周期)        ⚠️
    
  📌 已知问题：
  - field[22] 不是成交额，值(2-13万量级)与平安银行日成交额(~10亿)不匹配
  - field[11/14] 市值字段单位存疑，值偏小约10倍
  - 成交额改用估算: amount_est = 成交量(手) × 100 × 均价(反推)
...
class TdxStockStat:
    ...
    amount: float = 0.0             # field[22] — 不是成交额！待校准
    
    # 估算成交额 (从 流通市值×换手率 反推)
    # 公式: amount_est = float_mv × |turnover| / 100
    # 注意: float_mv 字段单位疑似被低估约10倍，加修正系数
    @property
    def amount_est(self) -> float:
        """估算个股成交额(万元) = 流通市值 × |换手率| / 100 × 10(单位修正)"""
        return self.float_mv * abs(self.turnover) / 100 * 10  # ×10 补偿单位偏低
