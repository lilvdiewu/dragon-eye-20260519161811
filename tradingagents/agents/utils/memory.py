...
class TradingMemoryLog:
    ...
    def __init__(self, config: dict = None):
        ...
        # 龙瞳注入：A股数据上下文（在 propagate 前设置）
        self._injected_a_stock_context = ""
    ...
    def get_past_context(self, ticker: str, n_same: int = 5, n_cross: int = 3) -> str:
        ...
        If _injected_a_stock_context is set (by dragon_eye), prepend it.
        """
        parts = []

        # 龙瞳注入的A股数据上下文（最优先）
        if self._injected_a_stock_context:
            parts.append(self._injected_a_stock_context)
    ...
    def inject_a_stock_context(self, ticker: str, context: str) -> None:
        """注入龙瞳A股数据上下文到 past_context

        在 TradingAgentsGraph.propagate() 调用前使用：
            ta.memory_log.inject_a_stock_context(ticker, a_stock_context)
            final_state, decision = ta.propagate(ticker, trade_date)

        context 会被放在 past_context 最前面，所有Agent都能看到。
        """
        self._injected_a_stock_context = context
